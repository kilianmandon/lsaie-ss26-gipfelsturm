...
'''
    ...
    job_name=f'gipfel-{mode}-{model_size}-{seq_len}seq-{mbs}mbs-{gbs}gbs-{nodes}n'
...
EXP_NAME={mode}-{model_size}-seq{seq_len}-mbs{mbs}-gbs{gbs}-${{SLURM_NNODES}}n
...
def profiling_and_offloading_args(config):
    ...
    if config['activation_offloading']:
        flags += [f'--cpu-offloading-num-layers {config["transformer_dimensions"]["num_layers"]-2}']
...
cd $MEGATRON_LM_DIR
flock $MEGATRON_LM_DIR/.git-lock bash -c "cd $MEGATRON_LM_DIR && git checkout -- . && git apply $WORKDIR/patches/0001-log-tokens-per-sec-to-wandb.patch"
